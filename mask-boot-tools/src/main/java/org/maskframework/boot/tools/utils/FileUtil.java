...
public class FileUtil {
    ...
    public static JSONObject createCode(String localPath, int size) {
        ...
        IntStream.range(0, size).forEach(t -> {
            ...
            g.setColor(c);
            g.drawString(String.valueOf(a), 5 + t * width / size, y);
        });
        //干扰线
        ...
        try {
            ...
            //验证码
            outcome.put("code", builder.toString());
            //路径
            outcome.put("msg", localPathFileName);
            //文件名
            outcome.put("fileName", fileName);
            return outcome;
        } catch (IOException e) {
            e.printStackTrace();
        }
    ...
    public static String getFileName(String filePath) {
        if (StringUtil.isEmptyEnhance(filePath)) {
            return filePath;
        }
    ...
    public static boolean writeString(File file, String content, boolean append, boolean endWithNewLine) {
        if (StringUtil.isEmptyEnhance(content) || !checkFileAndMakeDirs(file)) {
            return false;
        }
        //写入操作
